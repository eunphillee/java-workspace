...
public class ResultSetTest {
    ...
    public static void main(String[] args) {
        ...
        try (PreparedStatement pstmt =connection.prepareStatement(sql)) {
            ...
            ResultSet rs = pstmt.executeQuery();   // sselect 

            // 3) 조회된 행 집합 확인. 
            // 행집합 커서 (포인터)를 이용해서 한 개의 행만 읽어 옵니다. 
            //          다음 행을 읽어오려면 next() 메소드로 커서를 이동하고 boolean 리턴
            //              ㄴ 참 : 데이터 있음, 거짓 :데이터 없음
            //          커스의 초기상태는 -1 조회돈 첫번째 행 이전을 가리킵니다. 

            System.out.println("조회 결과가 있나요? ");
            System.out.println(rs.next());

            //rs 가 가리키는 행의 컬럼 값을 가져옵니다. 컬럼의 형식을 getXXX 설정합니다. 
            //rs.next() 가 false 이면 아래의 getxxx은 오류 입니다. 
            System.out.println("rs 가 가르키는 행의 컬럼값");
