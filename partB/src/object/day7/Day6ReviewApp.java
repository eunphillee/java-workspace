...
import object.day6.MyMathV3;

public class Day6ReviewApp {

    public static void main(String[] args) {
        // 새로운 클래스 Score로 데이터 저장과 관련 기능을 활용하는 에시.
        // 요구사항 -> 성적데이터 최대 10개로 성적관리 프로그램을 개발한다.  => 배열선언
        ...
        System.out.println("성적 데이터 출력2");
        for(int i=0;i<5;i++){
            String temp = stuScores[i].toScore();
            System.out.println(temp);
        }

        System.out.println("===== 저장된 객체들 중 점수만 출력하기 =======");
        for(int i=0;i<5;i++){
            // System.out.println(i +":" +stuScores[i].jumsu);  //jumsu는 private 필드- 오류
            System.out.println(i +":" +stuScores[i].getJumsu());
        }

        System.out.println("===== 저장된 객체들 중 '점수'가 85점 이사인 것만 과목명 ,점수 출력하기 =======");
...
// 요구사항 : 성적데이터에 이름, 학년, 과목명1개, 점수1개 저장 
//            -> 성적데이터 최대 10개로 성적관리 프로그램을 개발한다. 
//     => 클래스 설계는 개발자가 합니다.  (main 메소드에서 할일) 
class Score{
    //규칙: private 한 필드 선언 
    private String name; 
    ...
    public Score(String name, GradeType grade, String subject, int jumsu){
        this.name = name;
        //객체의 필드 name = 인자name  ==> 인자와 필드 변수명이 같을 대 필드는 this. 필드 필수
        this.grade = grade;
    ...
    public String toScore(){
     ...
     // return String.grade + ": 이름 = "+this.name",과목명 =" +this.subject+ ", 점수 = " +this.jumsu;
    }
}