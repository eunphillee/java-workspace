...
public class C05SetTest {
    public static void main(String[] args) {
        ...
        // add, remove, contains는 List 와 동일하게 사용함. 
        // indexOf, lastIndexOf 는 Set은 인덱스가 없어서 사용못함. 
        ...
        set.add("sana");
        System.out.println(set.size());  // 예측값 3-  중목된 값은 한번만 저장 

        // 1. hash 는 해시테이블. 해시테이블을 사용한 데이터 저장 
        System.out.println("HashSet 자료구조에 저장된 값 하나씩 가져오기 ");    
        for(String temp : set){          // 오직 향상for문으로만 가능 
            System.out.println(temp);    // temp 순서는 임의. 무작위 
        }


          // 2. Linked 는 연결된. 입력(추가)한 순서를 저장 (다음 항목의 주소를 저장하는 방식)
        ...
        for(String temp : linkSet)
         ...
         // 3. TreeSet : 이진트리 구조에 저장. (졍렬된 방식으로 접근.)
