...
public class C04PictureJumsu {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<photo.length;i++){             // 큰 배열  반복
            int sum = 0 ; // 각사진점수의 합계
            List<String> list = Arrays.asList(name);  //인덱스 검색을 위한 반환 
            // System.out.println(Arrays.toString(photo[i]));  // 큰 배열 출력 
            for(int j=0;j<photo[i].length;j++){   // 작은 배열 값 하나씩 출력 
...
class Test{
    ...
    void test(String[][] photo) {
        ...
        System.out.println("photo.length :" + photo[0].length);
        System.out.println("photo.length :" + photo[1].length);
        System.out.println("photo.length :" + photo[2].length);
