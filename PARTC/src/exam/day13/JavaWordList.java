...
public class JavaWordList implements WordList{
    ...
    private List<JavaWord> words;  //단어 저장 목록 
    ...
    //       한 개의 라인을 읽어서 ,로 분리 ->> 분리한 문자열을 각각 JavaWord 객체의 필드값으로 저정
    public void fileLoad() {    //이 메소드에서 모든 동작을 실행하도록 작성하세요.
        ...
        try {
            fr = new FileReader(this.filePath);  //C:\\Class241129\\연습.txt
            sc = new Scanner(fr);  
          
            while(sc.hasNext()){      
                line = sc.nextLine();    
                String[] temp = line.split(",");
                JavaWord item = new JavaWord(temp[0].trim(), temp[1].trim(),Integer.parseInt(temp[2].trim()));
                words.add(item);
    ...
    public void add(JavaWord word) {
        if(word.getEnglish() == null || word.getEnglish().length()==0){
            System.out.println("경고 : 영어단어는 한글자 이상 입력 하세요. ");
        }
  ...
  // 인자값 postion 위치부터 english 단어 찾기 :찾은 인덱스 리턴 
    ...
    public int indexOfWord(String english,int position){
        for(int i=position;i<words.size();i++){
            if(words.get(i).getEnglish().equals(english)){ // 리스트에 담긴 단어의 english 필드와 비교
